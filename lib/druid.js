...
/**
 * Same as {@link Client#exec} but chooses suitable node to exec query.
 *
 * @public
 * @param {Query} query Query instance
 * @param {function} callback Accepts arguments: (err, result)
 */
Druid.prototype.exec = function(query, callback) {
  ...
  if (!this.dataSources.hasOwnProperty(ds)) {
    return callback(new DruidError('No node serves data source: ' + ds))
  }

  var node = getLessLoaded(this, ds)
  node.exec(query, callback)
}
